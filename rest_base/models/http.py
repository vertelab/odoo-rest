    ...
    def decorator(f):
        ...
        def log_response(response):
            if routing.get('log', False):
                log_vals = {
                    "endpoint_url": ', '.join(url_endpoint for url_endpoint in routing.get('routes', False)),
                    "method": ', '.join(
                        url_endpoint for url_endpoint in routing.get('methods')
                    ) if routing.get('methods') else '',
                    "data": response,
                    "direction": "in",
                    "state": "ok",
                    "message": False,
                ...
                request.env['rest.api'].create_log(**log_vals)
