...
class KnowitController(http.Controller):
    ...
    @http.route(['/my/orders/<int:order_id>/sign_complete'], type='http', auth="public", methods=['POST'], csrf=False, website=True)
    def complete_signing(self, order_id, access_token, **res):
        _logger.error(res)
        html_form = base64.b64decode(res.get("EidSignResponse")).decode()
        _logger.error(html_form)
        data = {
        ...
        res = api_signport.signport_post(data, "/CompleteSigning")
    ...
    @http.route(['/my/orders/<int:order_id>/sign_start'], type='json', auth="public", website=True, methods=['POST'])
    def start_sign(self, order_id):


        data = json.loads(request.httprequest.data)
        _logger.warning(data)
        _logger.warning(f"access_token: {data.get('params', {}).get('access_token')}")
        ssn = data.get("params", {}).get("ssn")
        if not ssn:
            return False
        api_signport = self.get_signport_api()
        res = api_signport.post_sign_sale_order(ssn=ssn, order_id=order_id, access_token = data.get('params', {}).get('access_token'), message="Test message.")
        _logger.warning(f"res: {res}")
        res_json = json.dumps(res)
        return res_json
